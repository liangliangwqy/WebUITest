...
def getinfo(webaddress):  
    ...
    websitefilepath = os.path.abspath('.')+'/webaddress/'+webaddress#通过函数os.path.abspath得到当前程序所在的绝对路径，然后搭配用户所输入的网址得到用于存储下载网页的文件夹
    ...
    fobj1 = open(webaddress+'.txt','w')#在本目录下创建一个result.txt文件，里面存储最终得到的内容
...
if __name__=="__main__":  
    webaddress = input("请输入网址(不需要前缀 \"https://\"):")  
    getinfo(webaddress)  
